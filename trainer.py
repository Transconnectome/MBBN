...
from torch.nn import MSELoss,L1Loss,BCELoss, BCEWithLogitsLoss, Sigmoid, HuberLoss
...
class Trainer():
    ...
    def __init__(self,sets,**kwargs):
        ...
        if kwargs.get('task') == 'test':
            # for just test phase
            self.loaded_model_weights_path = kwargs.get('model_weights_path')
            print('self.loaded_model_weights_path', self.loaded_model_weights_path)
    ...
    def load_model_checkpoint(self):
        ...
        if self.transfer_learning:
            ...
            if len(pths) > 0 : # if there are any checkpoints from which we can resume the training. 
                ...
                self.state_dict = torch.load(os.path.join(self.experiment_folder,self.recent_pth),map_location='cpu', weights_only=False) #, map_location=self.device
                if self.transfer_learning:
                    self.model.load_partial_state_dict(self.state_dict['model_state_dict'],load_cls_embedding=False)
                else:    
    ...
    def forward_pass(self, input_dict):
        if self.flop_counter:
            ...
            sys.exit()
        # Determine forward function once at model init time
