...
class DataHandler():
            ...
            for i in sub:
                ...
                if file.shape[1] >= self.seq_len:
                    ...
                    for j in range(file.shape[0]):
                        if np.sum(file[j]) == 0:
                            ## All values for this ROI are zero — replace with mean
                            file[j] = np.array([np.mean(file)]*self.seq_len)
                            prob_sub.append(i)
    ...
    def get_params(self,eval=False,**kwargs):
        ...
        def worker_init_fn(worker_id):
            np.random.seed(np.random.get_state()[1][0] + worker_id)  # unique seed per worker
